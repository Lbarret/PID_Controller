...
int main() {
  ...
  std::cout << "Starting PID Controller with Kp = " << control.getKp() <<" Ki = " << control.getKi() << " Kd = " << control.getKd() << "TimeInterval" << control.getTimeInterval() <<std::endl;
  ...
  std::cout << "The New velocity using PID controller is = "
            << control.compute(target_velocity, actual_velocity) << std::endl;
  return 0;
}
